...
class TestTickerDaemonComplete:
    ...
    def mock_database_context(self):
        ...
        with patch('fullon_ticker_service.daemon.DatabaseContext') as mock:
            ...
            mock_db = AsyncMock()
            # Mock user lookup - MUST return a real string, not another AsyncMock
            mock_db.users.get_user_id = AsyncMock(return_value='admin_user')
            # Mock user exchanges (what daemon uses)
            mock_db.exchanges.get_user_exchanges = AsyncMock(return_value=[mock_user_exchange_1, mock_user_exchange_2])
            # Mock category exchanges (for name resolution)
            mock_db.exchanges.get_cat_exchanges = AsyncMock(return_value=[mock_cat_exchange_1, mock_cat_exchange_2])
            # Mock symbols retrieval
            mock_db.symbols.get_all = AsyncMock(side_effect=[
    ...
    async def test_process_registration(self, mock_database_context, mock_process_cache):
        ...
        with patch('fullon_orm.cache.cache_manager') as mock_cache_mgr:
            ...
            with patch('fullon_ticker_service.daemon.ExchangeHandler'):
                await daemon.start()
    ...
    async def test_empty_exchanges_handling(self):
        ...
        with patch('fullon_ticker_service.daemon.DatabaseContext') as mock_db_context:
            # Mock user lookup returning no admin user
            mock_db = AsyncMock()
            mock_db.users.get_user_id.return_value = None  # No admin user
            mock_db.exchanges.get_user_exchanges.return_value = []  # No exchanges
